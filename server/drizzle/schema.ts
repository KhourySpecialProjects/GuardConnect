import { pgTable, check, text, timestamp, integer, boolean, jsonb, uuid, vector, unique, primaryKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const careerStageEnum = pgEnum("career_stage_enum", ['new-soldiers', 'junior-ncos', 'senior-ncos', 'junior-officers', 'senior-officers', 'transitioning', 'no-preference'])
export const channelPostPermissionEnum = pgEnum("channel_post_permission_enum", ['admin', 'everyone', 'custom'])
export const matchStatusEnum = pgEnum("match_status_enum", ['pending', 'accepted', 'declined'])
export const meetingFormatEnum = pgEnum("meeting_format_enum", ['in-person', 'virtual', 'hybrid', 'no-preference'])
...
export const messageBlastStatusEnum = pgEnum("message_blast_status_enum", ['draft', 'sent', 'failed'])
...
export const reportCategoryEnum = pgEnum("report_category_enum", ['Communication', 'Mentorship', 'Training', 'Resources'])
export const reportStatusEnum = pgEnum("report_status_enum", ['Pending', 'Assigned', 'Resolved'])
export const roleNamespaceEnum = pgEnum("role_namespace_enum", ['global', 'channel', 'mentor', 'broadcast', 'reporting'])
...
export const channelSubscriptions = pgTable("channel_subscriptions", {
	subscriptionId: integer("subscription_id").primaryKey().generatedAlwaysAsIdentity({ name: "channel_subscriptions_subscription_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	userId: text("user_id").notNull(),
	...
	check("channel_subscriptions_notifications_enabled_not_null", sql`NOT NULL notifications_enabled`),
	check("channel_subscriptions_created_at_not_null", sql`NOT NULL created_at`),
]);

export const channels = pgTable("channels", {
	channelId: integer("channel_id").primaryKey().generatedAlwaysAsIdentity({ name: "channels_channel_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: text().notNull(),
	description: text(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	metadata: jsonb(),
	postPermissionLevel: channelPostPermissionEnum("post_permission_level").default('admin').notNull(),
}, (table) => [
...
export const inviteCodes = pgTable("invite_codes", {
	codeId: integer("code_id").primaryKey().generatedAlwaysAsIdentity({ name: "invite_codes_code_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	code: text().notNull(),
...
export const mentees = pgTable("mentees", {
	menteeId: integer("mentee_id").primaryKey().generatedAlwaysAsIdentity({ name: "mentees_mentee_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	userId: text("user_id").notNull(),
...
export const mentorRecommendations = pgTable("mentor_recommendations", {
	recommendationId: integer("recommendation_id").primaryKey().generatedAlwaysAsIdentity({ name: "mentor_recommendations_recommendation_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	userId: text("user_id").notNull(),
	...
	check("mentor_recommendations_recommended_mentor_ids_not_null", sql`NOT NULL recommended_mentor_ids`),
	check("mentor_recommendations_created_at_not_null", sql`NOT NULL created_at`),
]);

export const mentors = pgTable("mentors", {
	mentorId: integer("mentor_id").primaryKey().generatedAlwaysAsIdentity({ name: "mentors_mentor_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	userId: text("user_id").notNull(),
...
export const mentorshipEmbeddings = pgTable("mentorship_embeddings", {
	embeddingId: integer("embedding_id").primaryKey().generatedAlwaysAsIdentity({ name: "mentorship_embeddings_embedding_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	userId: text("user_id").notNull(),
...
export const mentorshipMatches = pgTable("mentorship_matches", {
	matchId: integer("match_id").primaryKey().generatedAlwaysAsIdentity({ name: "mentorship_matches_match_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
...
export const messageAttachments = pgTable("message_attachments", {
	attachmentId: integer("attachment_id").primaryKey().generatedAlwaysAsIdentity({ name: "message_attachments_attachment_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
...
export const messageBlasts = pgTable("message_blasts", {
	blastId: integer("blast_id").primaryKey().generatedAlwaysAsIdentity({ name: "message_blasts_blast_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	senderId: text("sender_id").notNull(),
	title: text().notNull(),
	content: text().notNull(),
	targetAudience: jsonb("target_audience"),
	sentAt: timestamp("sent_at", { withTimezone: true, mode: 'string' }),
	validUntil: timestamp("valid_until", { withTimezone: true, mode: 'string' }).default(sql`(now() + '24:00:00'::interval)`).notNull(),
...
export const pushSubscriptions = pgTable("push_subscriptions", {
	subscriptionId: integer("subscription_id").primaryKey().generatedAlwaysAsIdentity({ name: "push_subscriptions_subscription_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	userId: text("user_id").notNull(),
...
export const reportAttachments = pgTable("report_attachments", {
	attachmentId: integer("attachment_id").primaryKey().generatedAlwaysAsIdentity({ name: "report_attachments_attachment_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
...
export const roles = pgTable("roles", {
	roleId: integer("role_id").primaryKey().generatedAlwaysAsIdentity({ name: "roles_role_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
