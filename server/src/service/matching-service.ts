...
export class MatchingService {
  ...
  async createOrUpdateMentorEmbeddings(input: {
    ...
    try {
      const { userId } = input;
      const whyInterestedText = buildText(input.whyInterestedResponses, "mentor-why-interested");
  ...
  async createOrUpdateMenteeEmbeddings(input: {
    ...
    try {
      ...
      const hopeText = buildText(input.hopeToGainResponses, "mentee-hope-to-gain");
      const profileParts = [
