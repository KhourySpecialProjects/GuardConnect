...
const createMentee = procedure
  ...
  .mutation(async ({ input }) => {
    ...
    try {
      await matchingService.triggerMatchingForNewMentee(input.userId.toString());
      log.info("Matching process triggered successfully for new mentee", { menteeId: mentee.menteeId });
    } catch (error) {
